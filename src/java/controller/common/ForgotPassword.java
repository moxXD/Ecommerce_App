...
public class ForgotPassword extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        if (request.getSession().getAttribute("verifyStatus") != null && (int) request.getSession().getAttribute("verifyStatus") == 1) {
            request.getRequestDispatcher("authen/newPassword.jsp").forward(request, response);
            return;
        }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String OTP = EmailService.sendOTPResetPassword(user);
        if (OTP != null) {
